...
deferJsNs.DeferJs.prototype.runNext = function() {
  ...
  if (this.next_ < this.queue_.length) {
    ...
    try {
      ...
      console.error('Error when running the script: ' + e);
    }
  } else {
...
/**
  * This function is responsible for inserting in the right order tasks
  * that will execute an Element.insertBefore
  */
deferJsNs.DeferJs.prototype.submitTaskInOrder = function(task, pos) {
  var insertPos = this.taskOrderTable_[pos] || pos;

  this.submitTask(task, insertPos);

  if (this.taskOrderTable_[pos] == undefined) {
    this.taskOrderTable_[pos] = pos + 1;
  } else {
    this.taskOrderTable_[pos] += 1;
  }
};
