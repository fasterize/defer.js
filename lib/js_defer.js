...
deferJsNs.DeferJs = function() {
  ...
  this.origCreateElementNS_ = document.createElementNS;
   ...
   * @private
   */
  this._state_ = deferJsNs.DeferJs.STATES.NOT_STARTED;
  ...
  Object.defineProperty(this, 'state_', {
    ...
    set: function (newState) {
      switch(newState) {
        case deferJsNs.DeferJs.STATES.SCRIPTS_EXECUTING:
          this.mark('executing');
          break;
        case deferJsNs.DeferJs.STATES.SYNC_SCRIPTS_DONE:
          this.mark('sync_done');
          break;
      }
...
deferJsNs.DeferJs.prototype.mark = function(label) {
  window.performance && window.performance.mark && window.performance.mark('frz_deferjs_' + label);
}
...
deferJsNs.DeferJs.prototype.addStr = function(str, script_elem, opt_pos) {
  ...
  if (opt_pos) {
    this.log('Add to queue str at the position ' + opt_pos + ': ' + str);
  }
  else {
  ...
  this.submitTask(function() {
    ...
    var node = me.findJsNodeInDOM(script_elem);
...
deferJsNs.DeferJs.prototype.cloneScriptNode = function(opt_script_elem) {
  ...
  if (opt_script_elem) {
    ...
    for (var a = opt_script_elem.attributes, n = a.length, i = n - 1;
      ...
      // Ignore 'defer', as our current implementation has the same behavior.
 ...
 * @param {number} opt_pos Optional position for ordering.
 */
deferJsNs.DeferJs.prototype.addUrl = function(url, script_elem, opt_pos, encodeToUTF8) {
  if (opt_pos !== undefined){
    this.log('Insert to queue url at the position ' + opt_pos + ': ' + url);
  }
  else {
    this.log('Push to queue url: ' + url);
  }
  ...
  this.submitTask(function() {
    ...
    script.setAttribute('type', 'text/javascript');
    var asyncScript = script.hasAttribute('async');

    if (asyncScript) {
      var runNextAsyncHandler = function() {
        me.log('Executed async onload/onerror: ' + url);
        me.dynamicInsertedScriptCount_--;
        if (me.canCallOnComplete()) {
          me.onComplete();
        }
      };
      fasterizeutils.addHandler(script, 'load', runNextAsyncHandler);
      fasterizeutils.addHandler(script, 'error', runNextAsyncHandler);
    }
    else {
      var runNextHandler = function() {
        me.log('Executed onload/onerror: ' + url);
        me.runNext();
      };
    ...
    me.log('Inserting external script: ' + url);
    ...
    var currentElem = me.findJsNodeInDOM(script_elem);
...
deferJsNs.DeferJs.prototype.removeNotProcessedAttributeTillNode = function(
  ...
  if (document.querySelectorAll) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      var domNode = nodes.item(i);
      if (domNode == opt_node) {
        return;
      }
      domNode.removeAttribute(this.psaNotProcessed_);
    }
  }
};
...
deferJsNs.DeferJs.prototype.setNotProcessedAttributeForNodes = function() {
  ...
  for (var i = 0; i < nodes.length; i++) {
    var domNode = nodes.item(i);
    domNode.setAttribute(this.psaNotProcessed_, '');
  }
};
 ...
 */
deferJsNs.DeferJs.prototype.findJsNodeInDOM = function(scriptElem) {
  if (document.querySelector) {
    var scriptNodes = document.querySelectorAll(
        '[type="' + this.psaScriptType_ + '"]');
    var scriptClone = scriptElem.cloneNode();
    scriptClone.removeAttribute(this.psaNotProcessed_);
    for (var i = 0; i < scriptNodes.length; i++) {
      var domNode = scriptNodes.item(i);
      if (domNode.isEqualNode(scriptClone)) {
        return domNode;
      }
    }
  }
...
deferJsNs.DeferJs.prototype.setUp = function() {
  ...
  function addToDynamicInsertedScripts(elem, str) {
    if (me.overrideDefaultImplementation_ &&
  ...
  document.createElement = function(str) {
    ...
    addToDynamicInsertedScripts(elem, str);
    return elem;
  };

  // Overriding createElement().
  // Attaching onload & onerror function if script node is created.
  document.createElementNS = function(namespaceURI, str) {
    var elem = me.origCreateElementNS_.call(document, namespaceURI, str);
    addToDynamicInsertedScripts(elem, str);
    return elem;
  };
};
...
deferJsNs.DeferJs.prototype.fireEvent = function(evt) {
  ...
  this.log('Firing Event: ' + (evt === 1 ? 'DOM Content Loaded': 'Window Onload'));
...
var psaAddEventListener = function(elem, eventName, func,
  ...
  // - onreadystatechange and DOMContentLoaded is attached to Document as per spec
  // - onload can be attached on Window and Element. The event should never been invoked on Document
  // Ref : https://stackoverflow.com/questions/588040/window-onload-vs-document-onload
  ...
  } else if (elem !== document && deferJs.eventState_ < deferJsNs.DeferJs.EVENT.LOAD &&
...
deferJsNs.DeferJs.prototype.registerScriptTags = function() {
  ...
  for (var i = 0; i < len; ++i) {
    ...
    if (script.getAttribute('type') == this.psaScriptType_) {
      // async attribute is priority over defer attribute
      if (script.hasAttribute('async')) {
        this.addNode(script);
      }
      // sync scripts are placed before defer scripts and defer scripts are placed before async scripts
      else if (script.hasAttribute('defer')) {
        // add script with defer attribute after sync script
        this.addNode(script, syncScript + deferScript);
        deferScript++;
      }
      else {
        // add before deferjs script
        this.addNode(script, syncScript);
        syncScript++;
      }
    }
  }
...
deferJsNs.DeferJs.prototype.deferInsertion = function(newElement, insertFunction) {
  ...
  if (me.isJSNode(newElement) && newElement.getAttribute('src')) {
    ...
    if (me.state_ < deferJsNs.DeferJs.STATES.SYNC_SCRIPTS_DONE) {
      newElement.originalSrc = newElement.getAttribute('src');
      me.log('Defer insertion of ' + newElement.getAttribute('src'));
      me.submitTask(function() {
        newElement.currentSrc = newElement.getAttribute('src');
        newElement.setAttribute('src', newElement.originalSrc);
        ...
        try {
          ...
          newElement.setAttribute('src', newElement.currentSrc);
        }
        me.runNext();
      });
      return newElement;
