...
deferJsNs.DeferJs.prototype.addUrl = function(url, script_elem, opt_pos) {
  ...
  this.submitTask(function() {
    ...
    script.setAttribute('type', 'text/javascript');
    var asyncScript = script.getAttribute('async') === 'true' || script.getAttribute('async') === '';
...
deferJsNs.DeferJs.prototype.registerScriptTags = function() {
  ...
  for (var i = 0; i < len; ++i) {
    ...
    if (script.getAttribute('type') == this.psaScriptType_) {
      ...
      if (script.getAttribute('async') === 'true' || script.getAttribute('async') === '') {
        this.addNode(script);
      }
      // sync scripts are placed before defer scripts and defer scripts are placed before async scripts
      else if (script.getAttribute('defer') === 'true' || script.getAttribute('defer') === '') {
